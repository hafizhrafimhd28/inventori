...
if(isset($_POST['updatebarangmasuk'])){
    ...
    if($qty>$qtyskrg){
        ...
        $updatenya = mysqli_query($conn, "update brgmasuk set qty='$qty', ket='$ket', admin='$admin' where idmasuk='$idm'");
...
if(isset($_POST['konfirmasibarangmasuk'])){
    $idb = $_POST['idb'];
    $idm = $_POST['idm'];
    $admin = $_POST['admin'];
    $status = $_POST['status'];
    ...
    if($status==0){
        $konfirmasi = mysqli_query($conn, "update brgmasuk set status='1' where idmasuk='$idm'");
    }else{
        echo'<script>
        alert("Barang Sudah di Konfirmasi");
...
if(isset($_POST['updatebarangkeluar'])){
    ...
    if($qty>$qtyskrg){
        ...
        if($selisih<=$stocksekarang){
            $kurangistocknya = mysqli_query($conn, "update stockbrg set stock='$kurangin' where idbarang='$idb'");
            $updatenya = mysqli_query($conn, "update brgkeluar set qty='$qty', penerima='$penerima', admin='$admin' where idkeluar='$idk'");
...
if(isset($_POST['konfirmasibarangkeluar'])){
    $idb = $_POST['idb'];
    $idk = $_POST['idk'];
    $admin = $_POST['admin'];
    $status = $_POST['status'];
    ...
    if($status==0){
        $konfirmasi = mysqli_query($conn, "update brgkeluar set status='1' where idkeluar='$idk'");
    }else{
...
if(isset($_POST['tambahadmin'])){
    ...
    $addadmin = mysqli_query($conn, "INSERT INTO user (nama_lengkap, alamat, ttl, username, password, level) values('$namalengkap', '$alamat', '$ttl','$username','$password', '$level')");
    if($addadmin){
...
if(isset($_POST['updateadmin'])){
    ...
    $queryupdate=mysqli_query($conn, "update user set username='$usernamebr',nama_lengkap='$namalengkapbr',password='$passwordbr',ttl='$ttlbr',alamat='$alamatbr', level='$level' where id ='$id'");
    if($queryupdate){
        header('location:admin.php');
    }else{
        echo 'gagal input';
